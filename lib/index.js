...
Collector.prototype.toSVGPath = function Collector_toSVGPath(layers, transform) {
    ...
    for (i = 0; i < inserts.length; i++) {
        ...
        if (allLayers || layers.indexOf(insert.layer) !== -1) {
            var block = blocks[insert.blockName];
            if (block) {
                output += block.toSVGPath('*', {
                    scale: transform.scale,
                    translate_x: transform.translate_x - insert.x,
                    translate_y: transform.translate_y + insert.y,
                });
            }
        }
    }
...
Collector.prototype.toSVG = function Collector_toSVG(layers, transform, compact, asgroup) {
    ...
    if (!compact) {
        return svgString + ' width="100%" height="100%">' + nl + output + '</svg>';
    }
